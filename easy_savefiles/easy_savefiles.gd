...
static func get_project_settings(
	...
	for path in include:
		...
		for property in settings:
			...
			assert(property.has("hint"), "property dict has no <hint> key : %s"%JSON.stringify(property, '\t'))
			assert(property.has("hint_string"), "property dict has no <hint_string> key : %s"%JSON.stringify(property, '\t'))
			assert(property.has("usage"), "property dict has no <usage> key : %s"%JSON.stringify(property, '\t'))
			...
			assert(property["hint"] is int, "property dict's <hint> is not of type <int> : %s"%JSON.stringify(property, '\t'))
			assert(property["hint_string"] is String, "property dict's <hint_string> is not of type <String> : %s"%JSON.stringify(property, '\t'))
			assert(property["usage"] is int, "property dict's <usage> is not of type <int> : %s"%JSON.stringify(property, '\t'))
